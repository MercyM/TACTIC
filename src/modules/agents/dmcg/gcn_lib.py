...
class GTN(nn.Module):
    ...
    def forward(self, A, X, num_nodes=None, eval=False):
        ...
        for i in range(self.num_layers):
            ...
            print(f"[GTN Layer {i}] Layer time: {time.time() - start_layer:.6f}s")
        ...
        print(f"[GTN Forward] GCN total: {gcn_total:.6f}s, Linear: {linear_total:.6f}s, Total: {total_time:.6f}s")
...
class GTLayer(nn.Module):
    ...
    def forward(self, A, num_nodes, H_=None, eval=False):
        ...
        sparse_mm_time = time.time()

        print(
            f"[GTLayer Forward] Conv time: {conv_time - start_time:.6f}s, Sparse MM time: {sparse_mm_time - conv_time:.6f}s, Total: {sparse_mm_time - start_time:.6f}s")
...
class GTConv(nn.Module):
    ...
    def forward(self, A, num_nodes, eval=eval):
        ...
        print(f"[GTConv Forward] Total time: {total_time:.6f}s")
