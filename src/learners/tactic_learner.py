...
class TACTICLearner:
    ...
    def _get_adjacency_sequence(self, batch):
        """从模型前向传播中提取邻接矩阵序列"""
        adj_seq = []
        ...
        for t in range(batch.max_seq_length):
            ...
            if self.args.ifActionConstrast:
                f_i, f_ij, _ = self.mac.annotations(batch, t=t)
            else:
                f_i, f_ij = self.mac.annotations(batch, t=t)
            with torch.no_grad():
                adj = torch.sigmoid(f_ij)  # 将收益转换为概率
            adj_seq.append(adj)

        return torch.stack(adj_seq, dim=0)  # [seq_len, batch, agents, agents]

    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, seq_centroid=None, RBS_cluster=None,
              buffer_seq_labels=None, f_classifier=None):
        ...
        if min_timestep_true <= 10:
            checkpoint = 1
        mask_pred_accuracy = mask.expand_as(th.zeros([mask.size()[0], mask.size()[1], self.args.n_agents])).permute(0,
                                                                                                                    2,
                                                                                                                    1).reshape(
            -1, mask.size()[1])  # [bs,t,n_agents] --> [bs*n_agent,t]
        ...
        # seq sampling loop ==============================================================================
        for t in range(batch.max_seq_length):
            if self.args.mac == "tactic_mac" or self.args.mac == "lagmadcg_mac":
                if self.args.ifActionConstrast:
                    agent_outs, goal_prob_logits, _, contrast_loss = self.mac.forward(batch, t=t, goal_latent=None,
                                                                                      selected_goal=selected_goals[:,
                                                                                                    t].squeeze(
                                                                                          -1).reshape(-1),
                                                                                      compute_grads=True)
                else:
                    agent_outs, goal_prob_logits, _ = self.mac.forward(batch, t=t, goal_latent=None,
                                                                       selected_goal=selected_goals[:,
                                                                                     t].squeeze(
                                                                           -1).reshape(-1),
                                                                       compute_grads=True)
            ...
            if self.args.use_vqvae:
                ...
                if self.args.recon_type == 3:
                    timestep = th.tensor([float(t) / float(self.args.max_seq_length)]).repeat(
                        self.args.batch_size).unsqueeze(-1).to(self.args.device)
                ...
                if self.update_codebook and self.args.use_trj_Cqt == False:
                    self.vae.codebook_update(argmin, t_env, sum_rewards,
                                             reward_tgo[:, t])  # include for-batch
        ...
        if self.args.mac == "tactic_mac" or self.args.mac == "lagmadcg_mac":
            goal_prob_logits_out = th.stack(goal_prob_logits_out,
                                            dim=1)  # Concat over time [bs,t,n_agent,n_cluster]
        ...
        if seq_labels is not None:
            ...
            visit_sequences = self.default_node * th.ones((self.args.batch_size, self.args.max_seq_length)).to(
            ...
            if th.any(indices_not_clustered == True):
                if self.args.similarity_type <= 3:
                    if self.args.classifier_input_type == 1:  # full
                        ...
                        input_seq_raw = RBS_cluster.sequence_reduction(
                            visit_sequences[indices_not_clustered == True, :])
                        input_seq = self.default_node * th.ones((input_seq_raw.size()[0], ref_input_size)).to(
                            self.args.device).detach()
            ...
            seq_labels = seq_labels.unsqueeze(-1).unsqueeze(-1).expand(
                (self.bs, batch.max_seq_length - 1, self.args.n_agents)).to(th.long).to(self.args.device).detach()

            labels_reshaped = seq_labels.reshape(-1, 1).squeeze(-1)

            if self.args.mac == "tactic_mac" or self.args.mac == "lagmadcg_mac":
                goal_prob_logits_out_reshaped = goal_prob_logits_out[:, :batch.max_seq_length - 1].reshape(-1,
                                                                                                           self.args.n_cluster)
                ...
                # .. for monitoring only
                selected_goals_reshaped = selected_goals[:, :batch.max_seq_length - 1].squeeze(-1).permute(0, 2,
                                                                                                           1).reshape(
                    -1, batch.max_seq_length - 1).detach()  # [bs,t,n_agent,1] --> [bs*n_agents,t_max]

                goal_label_vec = seq_labels.permute(0, 2, 1).reshape(-1,
                                                                     batch.max_seq_length - 1).detach()  # [bs*n_agents,t_max]
        ...
        if self.args.use_vqvae:
            ...
            for t in range(batch.max_seq_length):
                ...
                if self.update_codebook and self.args.use_trj_Cqt == True:
                    self.vae.codebook_update_tdvq(argmin_t, t_env, sum_rewards, reward_tgo[:, t],
                                                  seq_labels=seq_labels)  # include for-batch
                ...
                if self.args.flag_batch_wise_vqvae_loss:
                    if self.args.recon_type == 1:
                        ...
                        vae_loss, ce_loss, vq_loss, commit_loss, coverage_loss = \
                            self.vae.loss_function_batch(state_input_t, recon_t, z_e_t, latent_emb_t, ndx=ndx,
                                                         Cqt=Cqt_target_t, recon_Cqt=Cqt_hat_t)
        ...
        for t in range(batch.max_seq_length):
            if self.args.mac == "tactic_mac" or self.args.mac == "lagmadcg_mac":
                if self.args.ifActionConstrast:
                    target_agent_outs, _, _, _ = self.target_mac.forward(batch, t=t, goal_latent=None,
                                                                         selected_goal=selected_goals[:,
                                                                                       t].squeeze(
                                                                             -1).reshape(-1))
                else:
                    target_agent_outs, _, _ = self.target_mac.forward(batch, t=t, goal_latent=None,
                                                                      selected_goal=selected_goals[:, t].squeeze(
                                                                          -1).reshape(-1))
            else:
        ...
        if self.args.double_q:
            ...
            if self.args.mixer == "dmaq_qatten":
                ...
                cur_max_actions_onehot = to_cuda(th.zeros(cur_max_actions.squeeze(3).shape + (self.args.n_actions,)),
                                                 self.args.device)
        ...
        # .. policy learning
        # 在损失计算部分添加以下代码--------------------------------
        # 假设通过模型获取邻接矩阵序列 [seq_len, batch, agents, agents]
        if self.args.ifSmooth:
            adjacency_matrices = self._get_adjacency_sequence(batch)

            smooth_loss = 0.0
            for t in range(1, adjacency_matrices.size(0)):  # 遍历时间步均
                # 修改计算部分
                diff = (adjacency_matrices[t] - adjacency_matrices[t - 1]).pow(2).sum(dim=(1, 2)).sqrt().mean()

                smooth_loss += self.args.lambda_smooth * diff
                del diff  # 显存优化

            # 修改总损失计算-----------------------------------------
            loss = loss + smooth_loss * self.args.lambda_smooth

        if self.args.ifActionConstrast:
            loss = loss + contrast_loss * self.args.lambda_contrast

        # 统计参数数量
        # total_params = sum(p.numel() for p in self.mac.parameters() if p.requires_grad)
        #
    ...
    def load_models(self, path):
        ...
        if self.args.use_vqvae:
            ...
            self.vae.emb.load_state_dict(
                th.load("{}/codebook.th".format(path), map_location=lambda storage, loc: storage))
