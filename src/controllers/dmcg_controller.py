...
class DeepCoordinationGraphMAC(BasicMAC):
    """ Based on Multi-agent controller for a Deep Coordination Graph (DCG, Boehmer et al., 2020)"""
    ...
    def __init__(self, scheme, groups, args):
        ...
        self.agent = GCNFeatureAgent(input_shape, self.args)
    ...
    def annotations(self, ep_batch, t, compute_grads=False, actions=None):
        ...
        with th.no_grad() if not compute_grads else contextlib.suppress():
            ...
            self.hidden_states = self.agent(agent_inputs, self.hidden_states)[1].view(ep_batch.batch_size,
                                                                                      self.n_agents, -1)
